 ...
 * Omni Core wallet.
 */
record ConsolidatedTransaction(Instant time, Sha256Hash txId, List<BitcoinTransactionInfo> outputs,
     ...
     */
    public ConsolidatedTransaction(List<BitcoinTransactionInfo> outputs, OmniTransactionInfo info) {
        this(Instant.ofEpochSecond(outputs.get(0).getTime()), outputs.get(0).getTxId(), outputs, info);
    }
}
